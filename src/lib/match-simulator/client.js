...
function showResults(result,nA,nB,cfg){
  ...
  // Ratings
  function buildRatings(rats,fk){const f=FORMATIONS[fk];return rats.map((r,i)=>{if(!r.p)return'';const sc=r.base.toFixed(1);const sv=parseFloat(sc);let cls='rs-avg';if(sv>=8.5)cls='rs-elite';else if(sv>=7.5)cls='rs-great';else if(sv>=6.8)cls='rs-good';else if(sv>=5.5)cls='rs-avg';else if(sv>=4.8)cls='rs-poor';else cls='rs-bad';const role=r.role||(f?f.pos[i]:r.p.r);const[bg,tc]=getPosBg(role);const rowCls='rating-row'+(r.sub?' sub-row':'')+(r.outMin?' off-row':'');const detail=r.sub?'<span class="rating-subdetail">Sub '+r.inMin+'&#39; for '+matchName(r.outFor)+'</span>':(r.outMin?'<span class="rating-subdetail">Replaced '+r.outMin+'&#39; by '+matchName(r.replacedBy)+'</span>':'');const eventText=(r.events||[]).filter(e=>!/^(IN|OUT) /.test(e)).join(' ');return'<div class="'+rowCls+'"><span class="rating-pos" style="background:'+bg+';color:'+tc+';">'+role+'</span><span class="rating-player"><span class="rating-name">'+r.p.n+'</span>'+detail+'</span><span class="rating-events">'+eventText+'</span><span class="rating-score '+cls+'">'+sc+'</span></div>';}).join('');}
